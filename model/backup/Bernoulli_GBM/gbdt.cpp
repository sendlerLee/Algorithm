...
{
    double PosCnt = std::accumulate(Y.begin(), Y.end(), 0.0);
    double NegCnt = Y.size() - PosCnt; 
    return static_cast<double>(log(PosCnt * 1.0 / NegCnt));
}
...
void CART::fit(Problem &prob, std::vector<double> &R, std::vector<double> &F_Tr, std::vector<double> &F1)
{
	...
	double pi = 1.0 / (1 + exp(-F_Tr[i]));
        ...
        tmp[tnode_idx].first += prob.Y[i] - pi;
...
void GBDT::write(Problem &prob, vector<double> & F_Val, std::string const &path)
{
	...
	double pi = 1.0 / (1 + exp(-val));
	//outfile << instance << "\t" << label << "\t1:1\t2:" << pi << std::endl;
	outfile << instance << "\t" << label << "\t" << pi << std::endl;
...
void GBDT::fit(Problem &Tr, Problem &Va)
{
    ...
    Timer timer;
    printf("iter	time	tr_llh	va_llh	tr_mse	va_mse\n");
    for(uint32_t t = 0; t < trees.size(); ++t)
    {
        ...
        #pragma omp parallel for schedule(static)
        for(uint32_t i = 0; i < Tr.nr_instance; ++i) 
            R[i] = static_cast<double>(Y[i] - 1.0 / (1 + exp(-F_Tr[i])));
        ...
        trees[t].fit(Tr, R, F_Tr, F1);

        double Tr_llh = 0;
        double Tr_mse = 0;
        #pragma omp parallel for schedule(static) reduction(+: Tr_llh, Tr_mse)
        for(uint32_t i = 0; i < Tr.nr_instance; ++i)
        {
            F_Tr[i] += learningrate * F1[i];
            Tr_llh += (Tr.Y[i] * F_Tr[i] - log(1+exp(F_Tr[i])));

	    double pi = 1.0 / (1 + exp(-F_Tr[i]));
	    Tr_mse += (Tr.Y[i] - pi) * (Tr.Y[i] - pi);
        }
        Tr_llh /= static_cast<double>(Tr.nr_instance);
        Tr_mse /= static_cast<double>(Tr.nr_instance);

        double Va_llh = 0;
        double Va_mse = 0;
        #pragma omp parallel for schedule(static) reduction(+: Va_llh, Va_mse)
        for(uint32_t i = 0; i < Va.nr_instance; ++i)
        {
            std::map<uint32_t, double> x = Va.inst2features[i];
            std::pair<uint32_t, double> res = trees[t].predict(x);
            F_Va[i] += learningrate * res.second;
            Va_llh += (Va.Y[i] * F_Va[i] - log(1+exp(F_Va[i])));

	    double pi = 1.0 / (1 + exp(-F_Va[i]));
	    Va_mse += (Va.Y[i] - pi) * (Va.Y[i] - pi);
        }
        Va_llh /= static_cast<double>(Va.nr_instance);
        Va_mse /= static_cast<double>(Va.nr_instance);

        printf("%4d %8.1f %10.8f %10.8f %10.8f %10.8f\n", t, timer.toc(), Tr_llh, Va_llh, Tr_mse, Va_mse);
...
uint32_t g_idx = 0;
void GBDT::plrrun(Problem& Tr, Problem& Va)
{
