...
{
public:
    void init(uint32_t depth, uint32_t minNodeCount)
    {
	min_tnode_capacity = minNodeCount;
	max_depth = depth;
...
{
public:
    RandomForest(uint32_t nr_tree, uint32_t nr_depth, uint32_t capacity)
    {
	trees.resize(nr_tree);
	max_depth = nr_depth;
	min_tnode_capacity = capacity;
    }
