...
struct Option
{
    Option() : nr_tree(30), nr_depth(5), nr_minnodecnt(50){}
...
int main(int const argc, char const * const * const argv)
{
    ...
    std::cout << "Train nr_instance: " << Tr.nr_instance << " nr_field: " << Tr.nr_field << std::endl;
    std::cout << "done\n" << std::flush;

    std::cout << "Reading test data...\n" << std::flush;
    read_test_data(Tr, Va, opt.Va_path);
    std::cout << "Test nr_instance: " << Va.nr_instance << " nr_field: " << Va.nr_field << std::endl;
    std::cout << "done\n" << std::flush;

    omp_set_num_threads(20);

    RandomForest randomforest(opt.nr_tree, opt.nr_depth, opt.nr_minnodecnt);
