...
int main(int const argc, char const * const * const argv) 
{
	...
	cout << "iter\ttr_loss\tva_loss\ttr_auc\tva_auc\ttr_mse\tva_mse" << endl;
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
        	...
        	for (int idx = 0; idx < trainidxvec.size(); idx += 1) 
		{
			...
			double pi = -yi / (1 + exp(yi * Fi));

			for (map<int, double>::iterator it = Tr.X[i].begin(); it != Tr.X[i].end(); ++it) 
			{
				int f = it->first;
				double x = it->second;
				Tr.W[f] -= opt.nr_lr * (pi * x + opt.nr_reg * Tr.W[f]);
			}
		}
		...
		cout << iter << "\t" << trainloss << "\t" << testloss << "\t" << trainauc << "\t" << testauc << "\t" << trainmse << "\t" << testmse << endl;
	}
