...
{    
	...
	int findex = useritem.find_last_of(":");
	string userid = useritem.substr(0, findex).c_str();
	string itemid = useritem.substr(findex + 1, useritem.size() - findex).c_str();
	...
	userid2TrainNameMap[idx] = useritem;
        idx += 1;
    }
}
