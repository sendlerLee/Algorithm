...
int main(int const argc, char const * const * const argv) 
{
	...
	cout << "iter\ttr_llh\tva_llh\ttr_auc\tva_auc\ttr_mse\tva_mse" << endl;
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
        	...
        	for (int i = 0; i < Tr.nr_instance; i += 1)
		{
			...
			double pi = yi - Fi > 0 ? -1 : 1;

			for (map<int, double>::iterator it = Tr.X[i].begin(); it != Tr.X[i].end(); ++it) 
			{
				int f = it->first;
				double x = it->second;
				double g = pi + opt.nr_reg * Tr.W[f];
				Tr.W[f] -= opt.nr_lr * g;
			}
		}
        	...
        	for (int i = 0; i < Tr.nr_instance; i += 1) 
		{
			...
			trainmse += (yi - Fi) * (yi - Fi);
			Tr.F[i] = Fi;

			//Tr.F[i] = 1.0 / (1 + exp(-Fi));
			trainllh += yi * Fi - log(1 + exp(Fi));
		}
		...
		cout << iter << "\t" << trainllh << "\t" << testllh << "\t" << trainauc << "\t" << testauc << "\t" << trainmse << "\t" << testmse << endl;
	}
