...
int main(int const argc, char const * const * const argv) 
{
	...
	F0 =  static_cast<double>(log(PosCnt * 1.0 / NegCnt));
	cout << "F0: " << F0 << endl;
  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
		...
		map<int, double> denominatormap;
		map<int, double> numeratormap;
		for (int i = 0; i < Tr.nr_instance; i += 1)
		{
            		int yi = Tr.Y[i];
			...
			for (map<int, double>::iterator it = Tr.X[i].begin(); it != Tr.X[i].end(); ++it) 
			{
				...
				double x = it->second;
                                ...
                                numeratormap[f] += qi * zi * x;
                                denominatormap[f] += qi * x * x;
			}
		}

		#pragma omp parallel for schedule(static)
		for (int i = 0;i < Tr.W.size(); i += 1)
                        Tr.W[i] -= opt.nr_lr * numeratormap[i] / (denominatormap[i] + opt.nr_reg);
        	...
        	for (int i = 0; i < Tr.nr_instance; i += 1) 
		{
			...
			Tr.F[i] = Fi + F0;
			double ctr = 1.0 / (1 + exp(- Tr.F[i]));
			//trainllh += yi * log(ctr) + (1 - yi) * log(1 - ctr);
			
			trainllh += yi * Tr.F[i] - log(1 + exp(Tr.F[i]));

			double p = 1.0 / (1 + exp(-Tr.F[i]));
			trainmse += (yi - p) * (yi - p);
		}
        	...
        	for (int i = 0; i < Va.nr_instance; i += 1) 
		{
			...
			Va.F[i] = Fi + F0;
			testllh += yi * Va.F[i] - log(1 + exp(Va.F[i]));
