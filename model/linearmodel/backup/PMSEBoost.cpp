...
int main(int const argc, char const * const * const argv) 
{
	...
	F0 =  static_cast<double>(PosCnt * 1.0 / Tr.Y.size());
	...
	cout << "Init F0:\t" << F0 << endl;
	cout << trainllh << "\t" << testllh << "\t" << trainmse << "\t" << testmse << endl;
  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
		...
		double trainllh = 0.0;
		double trainauc = 0.0;
		#pragma omp parallel for schedule(static) reduction(+: trainmse, trainllh, posscore)
        	for (int i = 0; i < Tr.nr_instance; i += 1) 
		{
            		int yi = Tr.Y[i];
			double Fi = 0.0;
		...
		trainmse /= Tr.nr_instance;
		//trainauc = calAUC(Tr);
		posscore /= Tr.nr_instance;

		double negscore = 0;
		double testmse = 0.0;
		double testllh = 0.0;
		double testauc = 0.0;
		#pragma omp parallel for schedule(static) reduction(+: testmse, testllh, negscore)
        	for (int i = 0; i < Va.nr_instance; i += 1) 
		{
            		int yi = Va.Y[i];
		...
		testllh /= Va.nr_instance;
		testmse /= Va.nr_instance;
		negscore /= Va.nr_instance; 

		//testauc = calAUC(Va);
		//writeWeightFile(Tr);

		cout << iter << "\t" << timer.toc() << "\t" << trainllh << "\t" << testllh << "\t" << trainmse << "\t" << testmse << "\tPosScore:\t" << posscore << "\tNegScore:\t" << negscore << endl;
	}
