...
void loadTrainInstance(Problem& Tr, ifstream& inputfile) 
{
	...
	while(getline(inputfile, line)) 
	{
		...
		int target = atoi(tokens[1].c_str());
		...
		for (size_t j = 2; j < tokens.size(); j += 2)
		{
			string feat = tokens[j];
			double x = atof(tokens[j+1].c_str());
...
int main(int const argc, char const * const * const argv) 
{
	...
	cout << "Load Data Finish, numTrainInstance:" << Tr.nr_instance << " numTrainFeature: " << Tr.nr_field << "\t" << timer2.toc() << endl;
