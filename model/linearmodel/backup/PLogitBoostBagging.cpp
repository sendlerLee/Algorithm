...
struct Option
{
    Option() : nr_iter(100), nr_lr(0.01), nr_reg(100), nr_spl(1.0), print_auc(0) {}
...
"-r <nr_reg>: set the reg default 100 \n"
"-a <print_auc>: print test auc default 0 \n"
"-s <nr_spl>: set negative sample rate default 1 \n");
}
...
int main(int const argc, char const * const * const argv) 
{
	...
	int* ofbvec = new int[Tr.nr_instance];
	...
	Timer timer;
	if (opt.print_auc == 1)
		cout << "iter\ttime\ttr_llh\tva_llh\ttr_mse\tva_mse\tva_auc" << endl;
	else 
  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
		...
		if (opt.print_auc == 1)
		{
			...
			cout << iter << "\t" << timer.toc() << "\t" << trainllh << "\t" << testllh << "\t" << trainmse << "\t" << testmse << "\t" << testauc << endl;
		}
		else 
			cout << iter << "\t" << timer.toc() << "\t" << trainllh << "\t" << testllh << "\t" << trainmse << "\t" << testmse << endl;
	}
