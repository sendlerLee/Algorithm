...
int main(int const argc, char const * const * const argv) 
{
  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
        	...
        	for (int i = 0; i < Tr.nr_instance; i += 1)
		{
			...
			double s = a + p + pidctrmap[ins.p].x;
		...
		double trainrmse = 0;
			...
			Instance& ins = Tr.X[i];
			int y = Tr.Y[i];
			//double s = pidctrmap[ins.p].x + userctrmap[ins.u].x + adsctrmap[ins.a].x + adsctrmap[ins.a].basectr;
			double s = pidctrmap[ins.p].x + adsctrmap[ins.a].x + pidctrmap[ins.p].basectr;
			trainrmse += (y - s) * (y - s);
