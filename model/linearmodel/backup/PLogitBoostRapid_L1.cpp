...
int main(int const argc, char const * const * const argv) 
{
	...
	double Cvr = PosCnt * 1.0 / Tr.Y.size();
	...
	cout << "Init Cvr:\t" << Cvr << endl;
	cout << trainllh << "\t" << testllh << "\t" << trainmse << "\t" << testmse << endl;

	Timer timer;
	cout << "iter\ttime\ttr_llh\tva_llh\ttr_mse\tva_mse\tsparsity" << endl;
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
		...
		cout << iter << "\t" << timer.toc() << "\t" << trainllh << "\t" << testllh << "\t" << trainmse << "\t" << testmse << "\t" << trimcnt * 1.0 / Tr.nr_field << endl;
	}
