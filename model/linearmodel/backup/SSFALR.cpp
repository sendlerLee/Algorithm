...
int main(int const argc, char const * const * const argv) 
{
	...
	double N_Pos = std::accumulate(Tr.Y.begin(), Tr.Y.end(), 0.0);
	double N_Neg = Tr.Y.size() - N_Pos;
  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
		...
		cout << "BestFeat: " << bestj << "\tFeat: " << Tr.id2featmap[bestj] << "\tWeight: " << g_selectFeatMap[bestj] << "\tError: " << minError << endl;

        	#pragma omp parallel for schedule(static)
		for (int i = 0; i < Tr.nr_instance; i += 1)
		{
			double yi = Tr.Y[i];
			if (Tr.X[i].count(bestj) != 0)
			{
				Tr.F[i] += opt.nr_lr * bestwgt * Tr.X[i][bestj];
