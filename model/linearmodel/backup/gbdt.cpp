...
void calc_bias(Problem& prob, map<uint32_t, double>& featweightmap)
{
	srand(time(NULL));
	for(map<uint32_t, string>::iterator it = prob.id2featmap.begin(); it != prob.id2featmap.end(); ++it) 
	{
		uint32_t fid = it->first;
		double weight = rand() % 10000 / 10000;
		weight = 2 * weight - 1;
		featweightmap[fid] = 0;
	}
	...
	for (uint32_t iter = 0; iter < nr_iter; iter += 1) 
	{
		...
		{
			double yi = (prob.Y[instid] + 1) / 2;

			double wxi = 0.0;
			map<uint32_t, double>& featuredict = prob.inst2features[instid];
		...
		llh /= prob.nr_instance;
		cout << "Iter: " << iter << "\t" << llh << endl;

	}
}

void linearpredict(Problem& prob, map<uint32_t, double>& featweightmap, vector<double>& predictval)
{
	for(uint32_t instid = 0; instid < prob.nr_instance; ++instid) 
	{
		double wxi = 0.0;
		...
		double pi = 1 / (1 + exp(- wxi));
		double val = 2 * pi - 1;
		predictval[instid] = val;
	}
}

double calc_gbm_bias(std::vector<double> const &Y)
{
...
{
    //bias = calc_gbm_bias(Tr.Y);
    //std::vector<double> F_Tr(Tr.nr_instance, bias), F_Va(Va.nr_instance, bias);
    
    map<uint32_t, double> featweightmap;
    calc_bias(Tr, featweightmap);
    vector<double> F_Tr(Tr.nr_instance), F_Va(Va.nr_instance);
    linearpredict(Tr, featweightmap, F_Tr);
    linearpredict(Va, featweightmap, F_Va);
	...
	//cout << "Tree: " << t << "\ttreedepth: " << treedepth << "\t" << trees[t].max_depth << "\t" << trees[t].max_tnodes << endl;
        trees[t].fit(Tr, R, F1);
