...
double calAUC(vector<pair<int, double> >& instancefxvec, int poscnt, map<int, int>& datalabel, map<int, string>& userid2NameMap)
{
	...
	for (size_t i = 0; i < instancefxvec.size(); ++i) 
	{
		...
		outfile << username << "\t" << label << "\t1:1\t2:" << fscore << endl;
		if (label == 1) 
		{
...
int main(int const argc, char const * const * const argv) 
{
	...
	int SampleCnt = opt.nr_sample;
        ...
        vector<int> trainidxvec(trainPosDataFeature.size());
        for (int idx = 0; idx < trainPosDataFeature.size(); idx += 1)
                trainidxvec[idx] = idx;
  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
		/*
                ...
                for (int i = 0; i < trainidxvec.size(); i += 1)
                {
                        int idx = trainidxvec[i];
		*/
		...
		*/
		...
		double posscore = 0.0;
		double trainrmse = 0;
        	#pragma omp parallel for schedule(static) reduction(+: trainrmse, posscore)
        	for (int idx = 0; idx < trainPosDataFeature.size(); ++idx)
		{
			double wxi = 0.0;
			map<string, double>& featuredict = trainPosDataFeature[idx];
			...
			trainrmse += pow(1 - wxi, 2);
			if (wxi > 0) posscore += wxi;
		}
		...
		{
			double wxi = 0.0;
			map<string, double>& featuredict = trainNegDataFeature[idx];
			for (map<string, double>::iterator fit = featuredict.begin(); fit != featuredict.end(); ++fit) 
			{
				string fid = fit->first;
				double fval = fit->second;
			...
			trainrmse += pow(0 - wxi, 2);
			if (wxi > 0) posscore += wxi;
		}
		trainrmse /= (trainPosDataFeature.size() + trainNegDataFeature.size());
		posscore /= (trainPosDataFeature.size() + trainNegDataFeature.size());

		double negscore = 0;
		double rmse = 0;
		vector<pair<int, double> > instancefxvec(testDataFeature.size());
        	#pragma omp parallel for schedule(static) reduction(+: rmse, negscore)
        	for (int idx = 0; idx < testDataFeature.size(); ++idx)
		{
			...
			rmse += pow(testDataLabel[idx] - wxi, 2);
			if (wxi > 0) negscore += wxi;
		}
		rmse /= testDataFeature.size();
		negscore /= testDataFeature.size();

		double auc = 0;
		//auc = calAUC(instancefxvec, g_pos_cnt, testDataLabel, userid2TestNameMap);
		cout << "Iter: " << iter << "\tAUC: " << auc << "\tTrainRMSE: " << trainrmse << "\tTestRMSE: " << rmse << "\tPosScore:\t" << posscore << "\tNegScore:\t" << negscore << endl;
	}
