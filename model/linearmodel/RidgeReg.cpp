  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
        	...
        	for (int idx = 0; idx < trainidxvec.size(); idx += 1) 
		{
			...
			for (map<int, double>::iterator it = Tr.X[i].begin(); it != Tr.X[i].end(); ++it) 
			{
				int f = it->first;
				double x = it->second;
				double g = pi * x + opt.nr_reg * Tr.W[f];
				Tr.W[f] -= opt.nr_lr * g;
			}
		}
        	...
        	for (int i = 0; i < Va.nr_instance; i += 1) 
		{
			...
			testrmse += pow(yi - Va.F[i], 2);
		}
		testrmse /= Va.nr_instance;

		double testauc = calAUC(Va);
		cout << "Iter: " << iter << "\tTrain RMSE: " << trainrmse << "\tTrainAUC: " << trainauc << "\tTest RMSE: " << testrmse << "\tTestAuc: " << testauc << endl;
	}
