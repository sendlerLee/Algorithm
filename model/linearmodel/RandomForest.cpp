...
int main(int const argc, char const * const * const argv) 
{
	...
	for (int f = 0; f < Tr.nr_field; f += 1) 
		sort(Tr.Xhat[f].begin(), Tr.Xhat[f].end(), mysortfunc);
  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
		...
		int instSample = 2;
		vector<bool> instbagvec(Tr.nr_instance);
        	for (int i = 0; i < Tr.nr_instance; i += 1) 
		{
        		bool ofb = static_cast<bool>(rand() % instSample);
			instbagvec[i] = ofb;
		}
		...
		for (int f = 0; f < Tr.nr_field; f += 1)
		{
        		bool ofb = static_cast<bool>(rand() % featSample);
			if (ofb == 0) continue;
