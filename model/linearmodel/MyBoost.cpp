...
struct Option
{
	Option() : nr_iter(100), nr_lr(0.01), lambda_2(100), print_auc(0), lambda_1(0.0), ZMax(200.0) {}
...
"-z <ZMax>: set ZMax param default 200 \n"
...
int main(int const argc, char const * const * const argv) 
{
	...
	double PosInitVal = Tr.nr_instance * 1.0 / PosCnt;
	double NegInitVal = Tr.nr_instance * 1.0 / NegCnt;
	...
	for (int i = 0; i < Tr.nr_instance; i += 1) 
	{
		...
		//if (yi == 1) Tr.Z[i] = 1 + exp(-Tr.F[i]);
		//if (yi == 0) Tr.Z[i] = - (1 + exp(Tr.F[i]));

		if (yi == 1) Tr.Z[i] = PosInitVal;
		if (yi == 0) Tr.Z[i] = - NegInitVal;

		if (ZMax > 0 && Tr.Z[i] > ZMax) Tr.Z[i] = ZMax;
		if (ZMax > 0 && Tr.Z[i] < - ZMax) Tr.Z[i] = -ZMax;
  	...
  	for (int iter = 0; iter < opt.nr_iter; iter += 1)
	{
		...
		if (opt.print_auc == 1 && opt.lambda_1 == 0)
		{
			...
			cout << iter << "\t" << timer.toc() << "\t" << tr_logloss << "\t" << va_logloss << "\t" << tr_mse << "\t" << va_mse << "\t" << vaauc << "\t" << sumposctr * 1.0 / sumposcnt << "\t" << sumnegctr * 1.0 / sumnegcnt<< endl;
		}
		...
		if (opt.print_auc == 0 && opt.lambda_1 == 0)
			...
			cout << iter << "\t" << timer.toc() << "\t" << tr_logloss << "\t" << va_logloss << "\t" << tr_mse << "\t" << va_mse << "\t" << sumposctr * 1.0 / sumposcnt << "\t" << sumnegctr * 1.0 / sumnegcnt << endl;
	}
