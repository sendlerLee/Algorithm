...
double calc_bias(std::vector<double> const &Y)
{
    ...
    return static_cast<double>(0.5 * log(PosCnt * 1.0 / NegCnt));
}
...
void CART::fit(Problem &prob, std::vector<double> &R, std::vector<double> &F1)
{
    ...
    */
	...
	//double pi = prob.Y[i] - r;
        ...
        tmp[tnode_idx].second += - r / (2 * prob.Y[i] - 1);
    }
...
void GBDT::fit(Problem &Tr, Problem &Va)
{
    ...
    for(uint32_t t = 0; t < trees.size(); ++t)
    {
        ...
        for(uint32_t i = 0; i < Tr.nr_instance; ++i) 
            ...
            //R[i] = Y[i] - 1.0 / (1 + exp(- F_Tr[i]));
            R[i] = -(2 * Y[i] - 1.0) * exp(-(2 * Y[i] - 1.0) * F_Tr[i]);
	    ...
	    //tr_logloss -= Tr.Y[i] * F_Tr[i] - log(1 + exp(F_Tr[i]));
	    tr_logloss += exp(-(2 * Tr.Y[i] - 1.0) * F_Tr[i]);
	    ...
	    //va_logloss -= Va.Y[i] * F_Va[i] - log(1 + exp(F_Va[i]));
	    va_logloss += exp(-(2 * Va.Y[i] - 1.0) * F_Va[i]);
