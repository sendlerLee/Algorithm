...
{
	//double SumVal = std::accumulate(Y.begin(), Y.end(), 0.0);
	//return static_cast<double>(SumVal * 1.0 / Y.size());
...
void CART::fit(Problem &prob, std::vector<double> &R, std::vector<double> &F1)
{
    ...
    {
        locations[i].r = R[i];
	if (rand() % 2 == 1) locations[i].ofb = true;
	//if (rand() % 5 == 1) locations[i].ofb = true;
    }
	
    map<uint32_t, uint32_t> selectFeatMap;

    double threshold = log10(prob.nr_field) * 1.0 / (log10(2) * prob.nr_field);
    threshold = 1;
    srandom(time(NULL));
    for(uint32_t fid = 0; fid < prob.nr_field; fid += 1)
    {
	double ofb = (rand() % 100000) * 1.0 / 100000;
	if (ofb < threshold) selectFeatMap[fid] = 1;
	else selectFeatMap[fid] = 0;
    }
...
void GBDT::write(Problem &prob, vector<double> & F_Val, std::string const &path)
{
	...
	double sij = exp(val) - 1;
	sij = val;
	if (sij < 0.00001) sij = 0;
	outfile << instance << "\t" << label << "\t" << sij << std::endl;
...
void GBDT::fit(Problem &Tr, Problem &Va)
{
    ...
    for(uint32_t i = 0; i < Tr.Y.size(); ++i) W[i] = 1.0;
    //bias = calc_bias(Tr.Y);
    bias = medianw(Tr.Y, Tr.W);

    Tr.F.resize(Tr.nr_instance, bias);
    Va.F.resize(Va.nr_instance, bias);
    ...
    {
	tr_logloss += abs(Tr.Y[i] - Tr.F[i]);
	
	double gap = Tr.Gap[i];
	double sij = exp(Tr.F[i]) - 1;
	sij = Tr.F[i];
	if (sij < 0) sij = 0;
    ...
    {
	va_logloss += abs(Va.Y[i] - Va.F[i]);

	double gap = Va.Gap[i];
	double sij = exp(Va.F[i]) - 1;
	sij = Va.F[i];
	if (sij < 0) sij = 0;
	...
	#pragma omp parallel for schedule(static)
	for(uint32_t i = 0; i < Tr.nr_instance; ++i) 
		if (Tr.Y[i] - Tr.F[i] > deta) R[i] = 1;
		else if (Tr.Y[i] - Tr.F[i] < deta) R[i] = -1;
		else R[i] = 0;
	...
	} else
	{
        	#pragma omp parallel for schedule(static)
        	for(uint32_t i = 0; i < Tr.nr_instance; ++i) 
	    		if (Tr.Y[i] - Tr.F[i] > 0) R[i] = 1;
	    		else if (Tr.Y[i] - Tr.F[i] < 0) R[i] = -1;
	    		else R[i] = 0;
	}
        ...
        for(uint32_t i = 0; i < Tr.nr_instance; ++i) 
	{
		uint32_t ofb = static_cast<uint32_t>(rand() % 10);
		if (ofb < 2) R[i] = 0 - R[i];
	}
	*/
	    ...
	    sij = Tr.F[i];
	    //if (sij < 0) sij = 0;
	    int predictgap = (int)(sij + 0.5);
	    	
	    //tr_loss += abs(gap - sij);
	    tr_loss += abs(gap - predictgap);
        }
        tr_logloss /= static_cast<double>(Tr.nr_instance);
        tr_loss /= static_cast<double>(Tr.nr_instance);
        ...
        for(uint32_t i = 0; i < Va.nr_instance; ++i)
        {
            ...
            Va.F[i] += learningrate * res.second;
	    va_logloss += abs(Va.Y[i] - Va.F[i]);
