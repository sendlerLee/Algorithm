...
class BaseModel(object):
    ...
    with tf.variable_scope(scope or "build_network"):
      with tf.variable_scope("decoder/output_projection"):
        self.output_layer = layers_core.Dense(
            2, use_bias=False, activation=tf.nn.sigmoid,name="output_projection")
  ...
  def _get_learning_rate_warmup(self, hparams):
    ...
    warmup_steps = 0 #hparams.warmup_steps
    warmup_scheme = "t2t"
  ...
  def build_graph(self, hparams, scope=None):
    ...
    with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
      ...
      logits = self.output_layer(encoder_state[-1][1])
      ## Loss
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        with tf.device("/cpu:0") :
          loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.cast(self.iterator.target,tf.float32),logits=tf.cast(logits,tf.float32)))

          pred = tf.equal(tf.argmax(logits,1),tf.argmax(self.iterator.target,1)) 
