...
double getIdealDCG(vector<pair<int, double> > rel) {
...
void swapChange(const vector<fm_sample>& samples,const vector<pair<int, double> >& estimates) {
    vector<pair<int,double> > rel;
    for(int i = 0; i < estimates.size(); i++) {
        int index = estimates[i].first;
        rel[i] = pair<int,double>(index,(double)samples[index].y);
    }
...
void ftrl_trainer::train(const vector<fm_sample>& samples)
{
    ...
    for(int k = 0; k < samples.size; k++) {
        ...
        estimateY[k] = pair<int,double>(k,p);
        factorSum[k] = sum;
    }

    sort(estimateY.begin(), estimateY.end(), mysortfunc);
    swapChange(samples,estimateY);
