...
map<int, map<int, double> > modelAccumulateWeight;
	...
	for (int mi = 0; mi < numModel; mi += 1) {
		for (map<int, double>::iterator fit = featureDict.begin(); fit != featureDict.end(); ++fit) {
			...
			model2featureDict[mi][fid] = 0.0;
                        modelAccumulateWeight[mi][fid] = 0.0;                     
		}
			...
			double sumright = 0.0;
				...
				double gradientcommon = 0 - wi * (zi - sumscore);
					...
					double gradient = gradientcommon * fval;
                                        modelAccumulateWeight[mi][fid] += gradient *  gradient;
					model2featureDict[mi][fid] -= learning_rate / sqrt(modelAccumulateWeight[mi][fid]) * (gradient + reg * model2featureDict[mi][fid]);
